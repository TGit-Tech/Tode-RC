...
//-----------------------------------------------------------------------------------------------------
const char* MenuName::Name() {                                        DBENTERL(("MenuName::Name[GET]"))
  if (CName != 0) return CName;
  if ( EENameAddress >= 0 ) { EEPROM.get(EENameAddress, FName); return FName+'\0'; }
  DBERRORAL(("CName==0 and EENameAddress<0 at : "),(EENameAddress)) 
  return "?Name?";
}
//-----------------------------------------------------------------------------------------------------
void MenuName::Name(const char* _Name) {                  DBENTERAL(("MenuName::Name[SET] = "),(_Name))
...
//-----------------------------------------------------------------------------------------------------
byte MenuName::Navigate(byte _Key) {
...
//-----------------------------------------------------------------------------------------------------
void MenuName::Show(byte _XPos, byte _YPos) {
  if (_XPos!=0) XPos=_XPos; if (_YPos!=0) YPos=_YPos;
  DisplayName();
}
//-----------------------------------------------------------------------------------------------------
byte MenuName::NavNone(byte _Key) {
  //if ( _Key==NAVDSPROW ) { DisplayRowName(_Row); return NAVKEYNONE; }
}
//-----------------------------------------------------------------------------------------------------
byte MenuName::NavName(byte _Key) {                         DBENTERAL(("MenuName::NavName"),(_Key,HEX))
  ...
  if ( _Key == NAVKEYOKAY ) {
    ...
    if ( ReturnKey!=0xFF ) return ReturnKey;
  }
  return _Key;
}
//-----------------------------------------------------------------------------------------------------
void MenuName::DisplayName() {
  ...
  Display->print(Name());
  if ( EENameAddress >= 0 ) { Display->SETCURSOR(VALCOL,YPos); Display->print("[NAME]"); }
}
//-----------------------------------------------------------------------------------------------------
byte MenuName::NavNameSet(byte _Key) {                   DBENTERAL(("MenuName::NavNameSet"),(_Key,HEX))
...
//-----------------------------------------------------------------------------------------------------
int MenuValue::Value() { DBENTERL(("MenuValue::Value GET")) return iValue; }
//-----------------------------------------------------------------------------------------------------
void MenuValue::Value(int _Value) { DBENTERAL(("MenuValue::Value[SET]"),(_Value)) iValue = _Value; }
//-----------------------------------------------------------------------------------------------------
byte MenuValue::Navigate(byte _Key) {          DBENTERAL(("MenuValue::Navigate"),(_Key,HEX))

  // SELECTOR
  if ( NavSelected == SEL_NONE ) { _Key = NavNone(_Key); }
  if ( NavSelected == SEL_NAME ) { 
    _Key = NavName(_Key);
    if (_Key==NAVKEYRIGHT) { NavSelected=SEL_VALUE; Show(); }
  }
  if ( NavSelected == SEL_SETNAME ) { _Key = NavNameSet(_Key); }
...
//-----------------------------------------------------------------------------------------------------
void MenuValue::Show(byte _XPos, byte _YPos) {
  if (_XPos!=0) XPos=_XPos; if (_YPos!=0) YPos=_YPos;
  DisplayName();DisplayValue();
}
//-----------------------------------------------------------------------------------------------------
byte MenuValue::NavNone(byte _Key) {
  //if ( _Key==NAVDSPROW ) { DisplayRowName(_Row); DisplayRowValue(_Row); return NAVKEYNONE; }
}
//-----------------------------------------------------------------------------------------------------
byte MenuValue::NavValue(byte _Key) {                     DBENTERAL(("MenuValue::NavValue"),(_Key,HEX))
  if ( _Key==NAVKEYLEFT ) { NavSelected=SEL_NAME; Show(); }
...
byte MenuValue::NavValueSet(byte _Key) {               DBENTERAL(("MenuValue::NavValueSet"),(_Key,HEX))
  ...
  if (_Key==NAVKEYLEFT) { 
    if ( VType&VTDIG ) {
      ...
      else { NavSelected=SEL_NAME; VDigitPos = 0; Show(); return NAVKEYNONE; }   // Else Move to Name
    } else {
      NavSelected=SEL_NAME; Show(); return NAVKEYNONE; // NO-SAVE and EXIT
    }
  }
  ...
  if ( _Key==NAVKEYUP || _Key==NAVKEYDOWN ) {
    ...
    if ( VType&VTHEX ) { 
      ...
      if (iSetTo>VMax) iSetTo=VMax; 
      if (iSetTo<VMin) iSetTo=VMin;
    }
...
//-----------------------------------------------------------------------------------------------------
int MenuEEValue::Value() {                                       DBENTERL(("MenuEEValue::Value() GET")) 
  if ( VType&VTBYTE ) return EEPROM.read(EEValAddress);
  EEPROM.get(EEValAddress, iValue);
  return iValue;
}
//-----------------------------------------------------------------------------------------------------
void MenuEEValue::Value(int _Value) {                   DBENTERAL(("MenuEEValue::Value[SET]"),(_Value))
  ...
  EEPROM.put(EEValAddress,iValue);                                  // Write Integer-Value
}
