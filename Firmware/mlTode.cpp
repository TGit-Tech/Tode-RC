...
//------------------------------------------------------------------------------------------------------
void AddTode::Value(int _Value) {
  iValue = _Value;
  if ( RF==0 ) { DBERRORL(("AddTode::Value(SET) RF==0")) return; }
  DBINFOAL(("AddTode::Value(SET): "),(iValue, HEX))
...
MenuItem* Tode::NewDevice(byte _DTKey) {                    DBENTERAL(("Tode::NewDevice"),(_DTKey,HEX))
  ...
  if (_RFID>29) { DBERRORL(("Tode::AddDevice OUT OF MEMORY")) return 0; }
...
//-----------------------------------------------------------------------------------------------------
MenuItem* Tode::AddDevice(byte _DTKey, byte _RFID) { DBENTERAAL(("Tode::AddDevice"),(_DTKey,HEX),(_RFID))
